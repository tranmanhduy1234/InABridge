...
class ImageEncoder(nn.Module):
    def __init__(self, return_layer: int = cfg.RETURN_LAYER, model_id=cfg.IMAGE_ENCODER_MODEL_ID) -> None:
        super().__init__()
...
if __name__ == "__main__":
    ...
    encoder = ImageEncoder().to(cfg.DEVICE)
    encoder.train()
    print("Compute grid shape: ", encoder.compute_grid_shape(cfg.IMAGE_SIZE, cfg.IMAGE_SIZE))
    ...
    x = torch.randn(2, 3, cfg.IMAGE_SIZE, cfg.IMAGE_SIZE, device=encoder.device)
    y = encoder(x)
    print("Input :", x.shape)
